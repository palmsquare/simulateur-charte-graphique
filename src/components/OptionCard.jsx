...
const OptionCard = ({ icon, title, description, selected, onClick, price }) => {
  return (
    <motion.div
      onClick={onClick}
      className={`relative cursor-pointer rounded-lg p-2 border transition-all ${
        selected
    ...
    >
      ...
      <h3 className={`font-bold text-sm mb-0.5 ${selected ? 'text-white' : 'text-black'}`}>{title}</h3>
      {description && <p className={`text-xs mb-1 ${selected ? 'text-white/90' : 'text-gray-600'}`}>{description}</p>}
      {price && (
        <div className={`text-xs font-semibold ${selected ? 'text-white' : 'text-black'}`}>
          {price}
        </div>
      )}
    </motion.div>
